...
parser.add_argument('--resume', '-r', default='1', type=str,
...
if args.method =='base':
    net = embed_net(n_class, no_local= 'on', gm_pool =  'off', arch=args.arch)
else:
...
if dataset == 'sysu':
    ...
    if len(args.resume) > 0:
        ...
        model_path = checkpoint_path + 'SYSU_indoor-search.t'
        print(model_path)
    ...
    print(
            'Evaluation:   Rank-1: {:.2%} | Rank-5: {:.2%} | Rank-10: {:.2%}| Rank-20: {:.2%}| mAP: {:.2%}'.format(
    ...
    for trial in range(1):
        test_trial = trial +1
        model_path = checkpoint_path + 'RegDB_Thermal-Visible.t'
        if os.path.isfile(model_path):
            print('==> loading checkpoint {}'.format(args.resume))
            checkpoint = torch.load(model_path)
            net.load_state_dict(checkpoint['net'])
        ...
        print(
            'Evaluation:   Rank-1: {:.2%} | Rank-5: {:.2%} | Rank-10: {:.2%}| Rank-20: {:.2%}| mAP: {:.2%}| mINP: {:.2%}'.format(
